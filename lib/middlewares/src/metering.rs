...
pub struct FunctionMetering<F: Fn(&Operator) -> (u8, u64) + Send + Sync> {
    ...
    /// ID generated by the timestamps it is created to keep track the local data it stores
    /// since this struct is run in parallel
    id: u128,
}
...
impl<F: Fn(&Operator) -> (u8, u64) + Send + Sync + 'static> ModuleMiddleware for Metering<F> {
    ...
    fn generate_function_middleware(&self, _: LocalFunctionIndex) -> Box<dyn FunctionMiddleware> {
        let id = time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let poex = self.poex.clone();
        poex.lock().unwrap().add_id(id);
...
impl<F: Fn(&Operator) -> (u8, u64) + Send + Sync> FunctionMiddleware for FunctionMetering<F> {
    fn feed<'a>(
        ...
        self.poex.lock().unwrap().add_opcode(cost.0, self.id);
        ...
        match operator {
            ...
            => {
                if self.accumulated_cost > 0 {
                    ...
                    self.poex.lock().unwrap().inject_poex_fn_at_the_end_of_block(state, self.id);
...
mod tests {
    ...
    impl PoEx for PoExMock {
        ...
        fn inject_poex_fn_at_the_end_of_block(
            &mut self,
