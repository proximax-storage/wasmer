...
use crate::poex_trait::PoExBuilder;
...
impl<'a, F: Fn(&Operator) -> (u8, u64) + Send + Sync> Metering<F> {
        ...
        match operator {
            ...
            => {
                if self.accumulated_cost > 0 {
                    ...
                    self.poex.inject_poex_fn_at_the_end_of_block(state);
...
mod tests {
    ...
    #[derive(Clone)]
    struct PoExBuilderMock {}

    struct PoExMock {}

    impl PoExBuilderMock {
        fn new() -> Self {
            Self {}
        }
    }
    ...
    impl PoExBuilder for PoExBuilderMock {
